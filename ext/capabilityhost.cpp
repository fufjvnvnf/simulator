...
CapabilityProcessingEvent::CapabilityProcessingEvent(double time,
                                                     CapabilityHost* h,
                                                     bool is_timeout)
...
SenderNotifyEvent::SenderNotifyEvent(double time, CapabilityHost* h)
    : Event(SENDER_NOTIFY, time) {
  this->host = h;
}
...
bool CapabilityFlowComparator::operator()(CapabilityFlow* a,
                                          CapabilityFlow* b) {
...
bool CapabilityFlowComparatorAtReceiver::operator()(CapabilityFlow* a,
                                                    CapabilityFlow* b) {
  ...
  if (params.deadline && params.schedule_by_deadline) {
    ...
    if (a->notified_num_flow_at_sender > b->notified_num_flow_at_sender)
      ...
      if (a->remaining_pkts() > b->remaining_pkts())
        ...
        return a->start_time >
               b->start_time;  // TODO: this is cheating. but not a big problem
      else
        return false;
    } else
      return false;
...
void CapabilityHost::start_capability_flow(CapabilityFlow* f) {
  if (debug_flow(f->id) || debug_host(this->id))
    std::cout << get_current_time() << " flow " << f->id << " src " << this->id
              << " curr q size " << this->queue->bytes_in_queue << " num flows "
              << this->active_receiving_flows.size() << "\n";

  this->active_sending_flows.push(f);
  f->send_rts_pkt();
  if (f->has_capability() &&
      ((CapabilityHost*)(f->src))->host_proc_event == NULL) {
...
void CapabilityHost::schedule_host_proc_evt() {
  ...
  this->host_proc_event =
      new HostProcessingEvent(qpe_time + td_time + INFINITESIMAL_TIME, this);
...
void CapabilityHost::schedule_capa_proc_evt(double time, bool is_timeout) {
  ...
  this->capa_proc_evt = new CapabilityProcessingEvent(
      get_current_time() + time + INFINITESIMAL_TIME, this, is_timeout);
  add_to_event_queue(this->capa_proc_evt);
}

void CapabilityHost::schedule_sender_notify_evt() {
  assert(this->sender_notify_evt == NULL);
  this->sender_notify_evt = new SenderNotifyEvent(
      get_current_time() + (params.get_full_pkt_tran_delay() * 20) +
          INFINITESIMAL_TIME,
      this);
...
void CapabilityHost::notify_flow_status() {
  ...
  while (!this->active_sending_flows.empty()) {
    ...
    if (!f->finished) {
      flows_tried.push(f);
      if (f->size_in_pkt > params.capability_initial) num_large_flow++;
    }
  }
  ...
  if (!this->active_sending_flows.empty()) this->schedule_sender_notify_evt();
}
...
void CapabilityHost::send_capability() {
  ...
  //    std::cout << get_current_time() << " CapabilityHost::send_capability()
  //    at host " << this->id << "\n";
  ...
  while (!this->active_receiving_flows.empty() && !capability_sent) {
    ...
    if (f->redundancy_ctrl_timeout > get_current_time()) {
      if (check_better_schedule(f)) could_schd_better = true;
    ...
    // ok to send
      ...
      if (f->capability_gap() > params.capability_window) {
        if (get_current_time() >=
            f->latest_cap_sent_time + params.capability_window_timeout *
                                          params.get_full_pkt_tran_delay())
          f->relax_capability_gap();
        else {
          if (f->latest_cap_sent_time + params.capability_window_timeout *
                                            params.get_full_pkt_tran_delay() <
              closet_timeout) {
            closet_timeout =
                f->latest_cap_sent_time + params.capability_window_timeout *
      ...
      if (f->capability_gap() <= params.capability_window) {
        ...
        if (f->capability_count == f->capability_goal) {
          f->redundancy_ctrl_timeout =
              get_current_time() + params.capability_resend_timeout *
                                       params.get_full_pkt_tran_delay();
        }
  ...
  {
    this->schedule_capa_proc_evt(params.get_full_pkt_tran_delay(1500 /* + 40*/),
                                 false);
  } else if (closet_timeout <
             999999)  // has unsend flow, but its within timeout
  {
  ...
  if (could_schd_better) this->could_better_schd_count++;
}
