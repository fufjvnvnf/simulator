...
CapabilityFlow::CapabilityFlow(uint32_t id, double start_time, uint32_t size,
                               Host* s, Host* d)
...
void CapabilityFlow::send_pending_data() {
  ...
  if (next_seq_no + mss <= this->size) {
    p = this->send(next_seq_no, capa_seq, capa_data_seq,
                   params.capability_third_level &&
                           this->size_in_pkt > params.capability_prio_thresh
                       ? 2
                       : 1);
    next_seq_no += mss;
  } else {
  ...
  if (debug_flow(this->id))
    std::cout << get_current_time() << " flow " << this->id << " send pkt "
              << this->total_pkt_sent << " " << p->size << "\n";

  double td = src->queue->get_transmission_delay(p->size);
  assert(((SchedulingHost*)src)->host_proc_event == NULL);
  ((SchedulingHost*)src)->host_proc_event = new HostProcessingEvent(
      get_current_time() + td + INFINITESIMAL_TIME, (SchedulingHost*)src);
...
void CapabilityFlow::receive(Packet* p) {
  ...
  if (p->type == NORMAL_PACKET) {
    ...
    if (packets_received.count(p->capa_data_seq) == 0) {
      ...
      received_count++;
      while (received_until < size_in_pkt &&
             packets_received.count(received_until) > 0) {
        received_until++;
      }
    }
...
Packet* CapabilityFlow::send(uint32_t seq, int capa_seq, int data_seq,
                             int priority) {
  ...
  Packet* p =
      new Packet(get_current_time(), this, seq, priority, pkt_size, src, dst);
  ...
  log->send_pkt(pkt_size - hdr_size, seq, get_current_time(), cwnd_mss);
  return p;
}
...
void CapabilityFlow::assign_init_capability() {
  ...
  for (int i = 0; i < init_capa; i++) {
    Capability* c = new Capability();
    c->timeout = get_current_time() +
                 init_capa * params.get_full_pkt_tran_delay() +
...
void CapabilityFlow::set_capability_count() {
  ...
  if (this->capability_count == this->capability_goal) {
    this->redundancy_ctrl_timeout =
        get_current_time() + init_capa * params.get_full_pkt_tran_delay() * 2;
  }
}
...
void CapabilityFlow::send_capability_pkt() {
  if (debug_flow(this->id))
    std::cout << get_current_time() << " flow " << this->id << " send capa "
              << this->capability_count << "\n";
  int data_seq_num = this->get_next_capa_seq_num();
  last_capa_data_seq_num_sent = data_seq_num;
  CapabilityPkt* cp = new CapabilityPkt(
      this, this->dst, this->src,
      params.capability_timeout * params.get_full_pkt_tran_delay(),
      this->remaining_pkts(), this->capability_count, data_seq_num);
  this->capability_count++;
  this->capability_packet_sent_count++;
  this->latest_cap_sent_time = get_current_time();
  add_to_event_queue(
      new PacketQueuingEvent(get_current_time(), cp, dst->queue));
}

void CapabilityFlow::send_notify_pkt(int num_flows_at_sender) {
  if (debug_flow(this->id))
    std::cout << get_current_time() << " flow " << this->id << " send notify "
              << num_flows_at_sender << "\n";
  StatusPkt* cp =
      new StatusPkt(this, this->src, this->dst, num_flows_at_sender);
  add_to_event_queue(
      new PacketQueuingEvent(get_current_time(), cp, src->queue));
}

void CapabilityFlow::send_rts_pkt() {
  RTS* rts = new RTS(this, this->src, this->dst, 0, 0);
  add_to_event_queue(
      new PacketQueuingEvent(get_current_time(), rts, src->queue));
}
...
bool CapabilityFlow::has_capability() {
  while (!this->capabilities.empty()) {
    ...
    if (this->capabilities.top()->timeout < get_current_time()) {
      if (CAPABILITY_MEASURE_WASTE) {
        this->capability_waste_count +=
            this->capabilities.top()->has_idle_sibling_sender ? 1 : 0;
      }
...
int CapabilityFlow::use_capability() {
  assert(!this->capabilities.empty() &&
         this->capabilities.top()->timeout >= get_current_time());
...
void CapabilityFlow::relax_capability_gap() {
  assert(this->capability_count - this->largest_cap_seq_received >= 0);
  this->largest_cap_seq_received =
      this->capability_count - params.capability_window;
}
