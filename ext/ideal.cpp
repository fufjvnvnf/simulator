...
void IdealArbiter::compute_schedule() {
  ...
  for (auto it = active_flows->begin(); it != active_flows->end(); it++) {
    ...
    if (!srcs->at(f->src->id) && !dsts->at(f->dst->id)) {
      ...
      if (previous != NULL && previous->id != f->id &&
      ...
      if (((IdealHost*)f->src)->host_proc_event == NULL ||
          ((IdealHost*)f->src)->host_proc_event->cancelled) {
        ((IdealHost*)f->src)->host_proc_event =
            new HostProcessingEvent(get_current_time(), (IdealHost*)f->src);
...
IdealHost::IdealHost(uint32_t id, double rate, uint32_t queue_type)
    : SchedulingHost(id, rate, queue_type) {
  this->dispatch = ideal_arbiter;
  this->active_flow = NULL;
}

void IdealHost::start(Flow* f) { dispatch->flow_arrival((IdealFlow*)f); }
...
void IdealHost::send() {
  ...
  if (active_flow != NULL && !active_flow->finished) {
    ...
    host_proc_event = new HostProcessingEvent(
        get_current_time() + td + INFINITESIMAL_TIME, this);
...
IdealAck::IdealAck(Flow* flow, uint32_t recv, Host* src, Host* dst)
    : PlainAck(flow, 0, params.hdr_size, src, dst) {
  received = recv;
}

IdealFlow::IdealFlow(uint32_t id, double start_time, uint32_t size, Host* s,
                     Host* d)
...
void IdealFlow::start_flow() { ((IdealHost*)src)->start(this); }
...
void IdealFlow::send_pending_data() {
  ...
  if (sent ==
      size) {  // if there was a timeout (sent > size), just wait for ACK
  ...
  switch (p->type) {
    ...
    case ACK_PACKET:
      ...
      if (this->acked >= size) {
        ...
        if (retx_event != NULL) {
          if (id == 21)
            std::cout << get_current_time() * 1e6 << " 21 finished??\n";
          cancel_retx_event();
        }
...
uint32_t IdealFlow::get_priority(uint32_t seq) { return size - acked; }
