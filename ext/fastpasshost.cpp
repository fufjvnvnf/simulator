...
FastpassHost::FastpassHost(uint32_t id, double rate, uint32_t queue_type)
    : Host(id, rate, queue_type, FASTPASS_HOST) {}
...
void FastpassHost::receive_schedule_pkt(FastpassSchedulePkt* pkt) {
  ...
  for (int i = 0; i < FASTPASS_EPOCH_PKTS; i++) {
    if (pkt->schedule->schedule[i])
      pkt->schedule->schedule[i]->schedule_send_pkt(
          pkt->schedule->start_time +
          i * params.fastpass_epoch_time / FASTPASS_EPOCH_PKTS);
  }

  delete pkt->schedule;
}

FastpassArbiter::FastpassArbiter(uint32_t id, double rate, uint32_t queue_type)
    : Host(id, rate, queue_type, FASTPASS_ARBITER) {}

void FastpassArbiter::start_arbiter() { this->schedule_proc_evt(1.0); }
...
std::map<int, FastpassFlow*> FastpassArbiter::schedule_timeslot() {
  ...
  while (!sending_flows.empty()) {
    ...
    if (debug_flow(f->id) && get_current_time() >= 1.03000023262123) {
      std::cout << get_current_time() << " attempting schedule flow " << f->id
                << " " << f->src->id << "->" << f->dst->id << " for epoch "
                << get_current_time() + params.fastpass_epoch_time << " s_free"
                << sender_free << " r_free" << receiver_free
                << " arb_remaining " << f->arbiter_remaining_num_pkts
                << " sender_last_pkt_sent " << f->sender_last_pkt_sent << "/"
                << f->size_in_pkt << "\n";
      if (!sender_free)
        std::cout << "sender used by " << schedule[f->src->id]->id << " "
                  << schedule[f->src->id]->src->id << "->"
                  << schedule[f->src->id]->dst->id << "\n";
    }
    ...
    if (f->arbiter_remaining_num_pkts > 0 && sender_free && receiver_free) {
      ...
      schedule[f->src->id] = f;
      if (debug_flow(f->id))
        std::cout << get_current_time() << " scheduled flow " << f->id
                  << " for epoch "
                  << get_current_time() + params.fastpass_epoch_time
...
void FastpassArbiter::schedule_epoch() {
  if (total_finished_flows >= params.num_flows_to_run) return;

  std::vector<FastpassEpochSchedule*> schedules;
  for (uint i = 0; i < params.num_hosts; i++) {
    schedules.push_back(new FastpassEpochSchedule(get_current_time() +
                                                  params.fastpass_epoch_time));
  }
  ...
  for (int i = 0; i < FASTPASS_EPOCH_PKTS; i++) {
    if (this->sending_flows.size() > 0) {
      ...
      for (auto iter = one_time_slot.begin(); iter != one_time_slot.end();
           ++iter) {
...
void FastpassArbiter::receive_rts(FastpassRTS* rts) {
  if (!((FastpassFlow*)rts->flow)->arbiter_received_rts) {
    ...
    dynamic_cast<FastpassTopology*>(topology)->arbiter->sending_flows.push(
        (FastpassFlow*)rts->flow);
  }
  ...
  if (rts->remaining_num_pkts < 0) {
    ...
    ((FastpassFlow*)rts->flow)->arbiter_remaining_num_pkts =
        rts->remaining_num_pkts;
}

FastpassFlowProcessingEvent::FastpassFlowProcessingEvent(double time,
                                                         FastpassFlow* f)
...
FastpassTimeoutEvent::FastpassTimeoutEvent(double time, FastpassFlow* f)
    : Event(FASTPASS_TIMEOUT, time) {
  this->flow = f;
}

FastpassTimeoutEvent::~FastpassTimeoutEvent() {}

void FastpassTimeoutEvent::process_event() { this->flow->fastpass_timeout(); }
