...
MagicHostScheduleEvent::MagicHostScheduleEvent(double time, MagicHost* h)
    : Event(MAGIC_HOST_SCHEDULE, time) {
  this->host = h;
}
...
MagicHost::MagicHost(uint32_t id, double rate, uint32_t queue_type)
...
void MagicHost::start(Flow* f) {
  if (debug_flow(f->id) || debug_host(this->id))
    std::cout << get_current_time() << " host:" << this->id << " flow started "
              << f->id << " " << f->src->id << "->" << f->dst->id
              << " sz:" << f->size_in_pkt << "\n";
...
void MagicHost::schedule() {
  if (debug_host(this->id))
    std::cout << get_current_time() << " host:" << this->id
              << " calling schedule()\n";
  ...
  if (this->flow_sending == NULL) {
    ...
    while (!active_sending_flows.empty()) {
      ...
      if (f->ack_timeout > get_current_time()) {
        if (f->ack_timeout < min_finish_time) min_finish_time = f->ack_timeout;
        flows_tried.push(f);
      } else if (((MagicHost*)(f->dst))->recv_busy_until <=
                     get_current_time() +
                         f->get_propa_time() * params.magic_trans_slack ||
                 f->size_in_pkt <
                     ((MagicHost*)(f->dst))->flow_receiving->size_in_pkt) {
        ...
        // if(debug_flow(f->id))
        //    std::cout << get_current_time() << "!!!!!!!!! host:" << f->src->id
        //    << " flow_sending setting to f " << f->id << "\n";

        ((MagicHost*)(f->src))->flow_sending = f;
        ((MagicHost*)(f->dst))->flow_receiving = f;
        int pkt_to_schd = std::max(
            (unsigned)1,
            std::min((unsigned)params.reauth_limit, f->remaining_pkt()));
        f->remaining_pkt_this_round = pkt_to_schd;
        ((MagicHost*)(f->dst))->recv_busy_until =
            get_current_time() + f->get_propa_time() + 0.0000012 * pkt_to_schd;
        scheduled = true;
        break;
      } else {
        f->virtual_rts_send_count++;
        if (((MagicHost*)(f->dst))->recv_busy_until < min_finish_time) {
          assert(((MagicHost*)(f->dst))->recv_busy_until -
                     f->get_propa_time() * params.magic_trans_slack >
                 get_current_time());
          min_finish_time = ((MagicHost*)(f->dst))->recv_busy_until -
                            f->get_propa_time() * params.magic_trans_slack;
        }
        ...
        if (params.magic_delay_scheduling) {
          double slack = ((MagicHost*)(f->dst))->recv_busy_until -
                         (get_current_time() +
                          f->get_propa_time() * params.magic_trans_slack);
          if (f->size_in_pkt < 10 &&
              slack < params.reauth_limit * 0.0000012 * 0.9) {
            has_short_flow_to_delay = true;
          }
          if (active_sending_flows.size() > 0 &&
              active_sending_flows.top()->size_in_pkt > 10 &&
              has_short_flow_to_delay)
            break;
        }
      }
    }
    ...
    if (!scheduled && !active_sending_flows.empty() &&
        min_finish_time < 999999) {
...
void MagicHost::send() {
  ...
  if (this->flow_sending && this->flow_sending->finished) {
    if (debug_host(this->id))
      std::cout << get_current_time() << " reschedule() at host " << this->id
                << " flow finished " << this->flow_sending->id << "\n";
    this->reschedule();
  }

  if (this->flow_sending &&
      ((MagicFlow*)(this->flow_sending))->remaining_pkt_this_round == 0) {
    if (((MagicFlow*)(this->flow_sending))->send_count >=
        (int)ceil(this->flow_sending->size_in_pkt * 1)) {
      ((MagicFlow*)(this->flow_sending))->ack_timeout =
          get_current_time() + 0.0000095;
    }
  ...
  if (this->queue->busy) {
    ...
    if (this->host_proc_event == NULL) {
      ...
      this->host_proc_event =
          new HostProcessingEvent(qpe->time + td + INFINITESIMAL_TIME, this);
    ...
    if (this->flow_sending) {
      if (debug_flow(this->flow_sending->id))
        std::cout << get_current_time() << " flow " << this->flow_sending->id
                  << " send pkt " << this->flow_sending->total_pkt_sent << "\n";
