...
void PFabricQueue::enque(Packet *packet) {
  ...
  if (bytes_in_queue > limit_bytes) {
    ...
    pkt_drop++;
    drop(worst_packet);
    logs::event::EventLog::packet("drop", worst_packet->log, get_current_time(),
                                  src->id, dst->id);
  }
}
...
Packet *PFabricQueue::deque() {
  if (bytes_in_queue > 0) {
    ...
    if (p->type == NORMAL_PACKET) {
      ...
      if (this->location == 0) p->flow->first_hop_departure++;
      if (this->location == 3) p->flow->last_hop_departure++;
    }
    return p;
