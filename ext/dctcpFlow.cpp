...
DctcpFlow::DctcpFlow(uint32_t id, double start_time, uint32_t size, Host *s,
                     Host *d)
...
p->ecn, delayed_ack_counter); add_to_event_queue(new
PacketQueuingEvent(get_current_time(), a, dst->queue));
}
...
void DctcpFlow::receive_ack(Ack *a) {
  ...
  if (ack > last_unacked_seq) {
    ...
    // Update ecn_history
    // for (uint32_t i = 0; i < dca->delayed_num; i++) // only needed for
    // delayed ack
...
uint32_t DctcpFlow::get_priority(uint32_t seq) { return 1; }
