...
HostProcessingEvent::HostProcessingEvent(double time, SchedulingHost* h)
    : Event(HOST_PROCESSING, time) {
  this->host = h;
}
...
SchedulingHost::SchedulingHost(uint32_t id, double rate, uint32_t queue_type)
    : Host(id, rate, queue_type, SCHEDULING_HOST) {
...
void SchedulingHost::start(Flow* f) {
  ...
  if (this->host_proc_event == NULL ||
      this->host_proc_event->time < get_current_time()) {
    this->send();
...
void SchedulingHost::send() {
  ...
  if (!this->queue->busy) {
    while (!this->sending_flows.empty() &&
           (this->sending_flows.top())->finished) {
