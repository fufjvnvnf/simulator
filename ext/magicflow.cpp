...
MagicFlow::MagicFlow(uint32_t id, double start_time, uint32_t size, Host *s,
                     Host *d)
...
Packet *MagicFlow::send(uint32_t seq) {
  uint32_t priority = 1;
  if (this->size_in_pkt > 8) priority = 2;
  // priority = this->remaining_pkt();
  Packet *p = new Packet(get_current_time(), this, seq, priority,
                         mss + hdr_size, src, dst);
  total_pkt_sent++;
  add_to_event_queue(new PacketQueuingEvent(get_current_time(), p, src->queue));
  log->send_pkt(mss, seq, get_current_time(), cwnd_mss);
  return p;
}
...
void MagicFlow::send_pending_data() {
  ...
  if (((SchedulingHost *)src)->host_proc_event == NULL ||
      ((MagicHost *)src)->is_host_proc_event_a_timeout) {
    ...
    ((SchedulingHost *)src)->host_proc_event =
        new HostProcessingEvent(get_current_time() + td, (SchedulingHost *)src);
...
void MagicFlow::receive(Packet *p) {
  ...
  if (p->type == NORMAL_PACKET) {
    ...
    //            assert( this ==
    //            ((QuickSchedulingHost*)(this->dst))->flow_receiving );
