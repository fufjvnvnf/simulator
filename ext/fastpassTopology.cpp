...
FastpassAggSwitch::FastpassAggSwitch(uint32_t id, uint32_t nq1, double r1,
                                     uint32_t nq2, double r2,
                                     uint32_t queue_type)
    : AggSwitch(id, nq1, r1, nq2, r2, queue_type) {
  queue_to_arbiter = NULL;
}

FastpassTopology::FastpassTopology(uint32_t num_hosts,
                                   uint32_t num_agg_switches,
                                   uint32_t num_core_switches, double bandwidth,
                                   uint32_t queue_type)
    : PFabricTopology(num_hosts, num_agg_switches, num_core_switches, bandwidth,
                      queue_type) {
  ...
  for (uint32_t i = 0; i < num_agg_switches; i++) {
    AggSwitch *sw;
    if (i == 0)
      sw = new FastpassAggSwitch(i, hosts_per_agg_switch, c1, num_core_switches,
                                 c2, queue_type);
    else
      sw = new AggSwitch(i, hosts_per_agg_switch, c1, num_core_switches, c2,
                         queue_type);
  ...
  for (uint32_t i = 0; i < num_core_switches; i++) {
    CoreSwitch *sw =
        new CoreSwitch(i + num_agg_switches, num_agg_switches, c2, queue_type);
    core_switches.push_back(sw);
    switches.push_back(sw);
  }
  ((FastpassAggSwitch *)agg_switches[0])->queue_to_arbiter =
      Factory::get_queue(num_agg_switches + num_core_switches, c1,
                         params.queue_size, queue_type, 0, 3);
  ...
  // std::cout << "Linking arbiter with queue " << arbiter->queue->id << " " <<
  // arbiter->queue->unique_id << " with agg switch " << agg_switches[0]->id <<
  // "\n" ;
  ...
  for (uint32_t i = 0; i < num_agg_switches; i++) {
    ...
    for (uint32_t j = 0; j < hosts_per_agg_switch; j++) {  // TODO make generic
      ...
      // std::cout << "Linking Agg " << i << " to Host" << i * 16 + j << " with
      // queue " << q->id << " " << q->unique_id << "\n";
    }
    // Queues to Core
    for (uint32_t j = 0; j < num_core_switches; j++) {
      Queue *q = agg_switches[i]->queues[j + 16];
      q->set_src_dst(agg_switches[i], core_switches[j]);
      // std::cout << "Linking Agg " << i << " to Core" << j << " with queue "
      // << q->id << " " << q->unique_id << "\n";
    }
  }

  ((FastpassAggSwitch *)agg_switches[0])
      ->queue_to_arbiter->set_src_dst(agg_switches[0], arbiter);
  // std::cout << "Linking arbiter switch " << agg_switches[0]->id << " with
  // queue " << ((FastpassAggSwitch*) agg_switches[0])->queue_to_arbiter->id <<
  // " " << ((FastpassAggSwitch*) agg_switches[0])->queue_to_arbiter->unique_id
  // << "\n";
  ...
  for (uint32_t i = 0; i < num_core_switches; i++) {
    for (uint32_t j = 0; j < num_agg_switches; j++) {
      ...
      // std::cout << "Linking Core " << i << " to Agg" << j << " with queue "
      // << q->id << " " << q->unique_id << "\n";
    }
  }
