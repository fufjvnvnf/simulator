...
void FastpassFlow::start_flow() { update_remaining_size(); }

void FastpassFlow::update_remaining_size() {
  FastpassRTS* rts = new FastpassRTS(
      this, this->src, dynamic_cast<FastpassTopology*>(topology)->arbiter,
...
void FastpassFlow::send_schedule_pkt(FastpassEpochSchedule* schd) {
  FastpassSchedulePkt* pkt = new FastpassSchedulePkt(
      this, dynamic_cast<FastpassTopology*>(topology)->arbiter, this->src,
      schd);
  add_to_event_queue(new PacketQueuingEvent(
      get_current_time(), pkt,
      dynamic_cast<FastpassTopology*>(topology)->arbiter->queue));
}
...
int FastpassFlow::next_pkt_to_send() {
  ...
  for (int i = 0; i < this->size_in_pkt; i++) {
    pkt++;
    if (pkt >= this->size_in_pkt) pkt = sender_acked_until;
    if (sender_acked.count(pkt) == 0) return pkt;
  }
}
...
void FastpassFlow::send_data_pkt() {
  ...
  Packet* p =
      new Packet(get_current_time(), this, this->sender_last_pkt_sent * mss, 1,
                 mss + hdr_size, src, dst);
  if (debug_flow(this->id))
    std::cout << get_current_time() << " flow " << this->id << " send data "
              << this->sender_last_pkt_sent << " \n";
  total_pkt_sent++;
  next_seq_no += mss;
...
void FastpassFlow::fastpass_timeout() {
  if (!this->sender_finished) {
    this->sender_remaining_num_pkts =
        this->size_in_pkt - this->sender_acked_count;
  ...
  if (p->type == FASTPASS_RTS) {
    if (debug_flow(this->id))
      std::cout << get_current_time() << " flow " << this->id
                << " received rts\n";
    dynamic_cast<FastpassTopology*>(topology)->arbiter->receive_rts(
        (FastpassRTS*)p);
...
ArbiterProcessingEvent::ArbiterProcessingEvent(double time, FastpassArbiter* a)
    : Event(ARBITER_PROCESSING, time) {
  this->arbiter = a;
}
