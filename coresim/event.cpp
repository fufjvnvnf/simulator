...
extern std::priority_queue<Event *, std::vector<Event *>, EventComparator>
    event_queue;
...
FlowCreationForInitializationEvent::FlowCreationForInitializationEvent(
    double time, Host *src, Host *dst, EmpiricalRandomVariable *nv_bytes,
...
void FlowCreationForInitializationEvent::process_event() {
  ...
  if (params.bytes_mode) {
    ...
    nvVal = (nv_bytes->value() +
             0.5);  // truncate(val + 0.5) equivalent to round to nearest int
    if (nvVal > 2500000) {
      std::cout << "Giant Flow! event.cpp::FlowCreation:" << 1000000.0 * time
                << " Generating new flow " << id << " of size "
                << (nvVal * 1460) << " between " << src->id << " " << dst->id
                << "\n";
      nvVal = 2500000;
    }
  ...
  //        std::cout << "event.cpp::FlowCreation:" << 1000000.0 * time << "
  //        Generating new flow " << id << " of size "
  //         << size << " between " << src->id << " " << dst->id << " " <<
  //         (tnext - get_current_time())*1e6 << "\n";

  add_to_event_queue(new FlowCreationForInitializationEvent(
      tnext, src, dst, nv_bytes, nv_intarr));
}
...
void FlowArrivalEvent::process_event() {
  ...
  if (params.num_flows_to_run > 10 && flow_arrival_count % 100000 == 0) {
    ...
    std::cout << "## " << current_time << " NumPacketOutstanding "
              << num_outstanding_packets << " NumUnfinishedFlows "
              << num_unfinished_flows << " StartedFlows " << flow_arrival_count
...
void PacketQueuingEvent::process_event() {
  if (!queue->busy) {
    queue->queue_proc_event =
        new QueueProcessingEvent(get_current_time(), queue);
    add_to_event_queue(queue->queue_proc_event);
    queue->busy = true;
    queue->packet_transmitting = packet;
  } else if (params.preemptive_queue &&
             this->packet->pf_priority <
                 queue->packet_transmitting->pf_priority) {
    double remaining_percentage =
        (queue->queue_proc_event->time - get_current_time()) /
        queue->get_transmission_delay(queue->packet_transmitting->size);
...
void QueueProcessingEvent::process_event() {
  ...
  if (packet) {
    ...
    queue->packet_transmitting = packet;
    logs::event::EventLog::packet("receive", packet->log, get_current_time(),
    ...
    if (next_hop == NULL) {
      ...
      logs::event::EventLog::packet("arrive", packet->log, get_current_time(),
                                    queue->src->id, queue->dst->id);
      ...
      if (params.cut_through == 1) {
        ...
        queuing_evt = new PacketQueuingEvent(time + cut_through_delay + pd,
                                             packet, next_hop);
      } else {
...
void FlowFinishedEvent::process_event() {
  ...
  this->flow->log->end(true, active_flows, this->flow->finish_time,
                       this->flow->cwnd_mss, slowdown);
  if (slowdown < 1.0 && slowdown > 0.9999) {
    slowdown = 1.0;
  }
  if (slowdown < 1.0) {
    std::cout << "bad slowdown " << 1e6 * flow->flow_completion_time << " "
  ...
  if (print_flow_result()) {
    ...
    std::cout << flow->id << " " << flow->size << " " << flow->src->id << " "
              ...
              << flow->received_count << " " << flow->data_pkt_drop << "/"
              << flow->ack_pkt_drop << "/" << flow->pkt_drop << " "
...
void FlowProcessingEvent::process_event() { this->flow->send_pending_data(); }
...
void RetxTimeoutEvent::process_event() { flow->handle_timeout(); }
