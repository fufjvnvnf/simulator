...
int EmpiricalRandomVariable::loadCDF(std::string filename) {
  ...
  while (std::getline(myfile, line)) {
    ...
    double flow_sz = this->smooth ? (table_[numEntry_].val_ + prev_sz) / 2.0
                                  : table_[numEntry_].val_;
    assert(freq >= 0);
    w_sum += freq * flow_sz;
  ...
  // std::cout << "Mean flow size derived from CDF file:" <<
  // this->mean_flow_size << " smooth = " << this->smooth << "\n"; std::cout <<
  // "Number of lines in text file: " << numEntry_ << "\n";
  if (myfile.is_open()) {
    myfile.close();
  }
  return numEntry_;
...
EmpiricalBytesRandomVariable::EmpiricalBytesRandomVariable(std::string filename,
                                                           bool smooth)
    : EmpiricalRandomVariable("", smooth) {
  if (filename != "") loadCDF(filename);
}
...
double CDFRandomVariable::value() {
  ...
  //  std::cout << " notfound " << table_[numEntry_-1].cdf_ << " " <<
  //  table_[numEntry_-1].val_ << "\n";
  return table_[numEntry_ - 1].val_;
}

ConstantVariable::ConstantVariable(double value)
    : EmpiricalRandomVariable("", false) {
  this->v = value;
}

double ConstantVariable::value() { return v; }
...
double GaussianRandomVariable::value() { return (*distribution)(generator); }
