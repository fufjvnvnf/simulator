...
extern void read_experiment_parameters(std::string conf_filename,
                                       uint32_t exp_type);
extern void read_flows_to_schedule(std::string filename, uint32_t num_lines,
                                   Topology *topo);
...
void validate_flow(Flow *f) {
  double slowdown =
      1000000.0 * f->flow_completion_time / topology->get_oracle_fct(f);
  if (slowdown < 0.999999) {
    std::cout << "Flow " << f->id << " has slowdown " << slowdown << "\n";
    // assert(false);
  }
  // if(f->first_byte_send_time < 0 || f->first_byte_send_time < f->start_time -
  // INFINITESIMAL_TIME)
  //    std::cout << "Flow " << f->id << " first_byte_send_time: " <<
  //    f->first_byte_send_time << " start time:"
  //        << f->start_time << "\n";
}
...
void debug_flow_stats(std::deque<Flow *> flows) {
  ...
  for (uint32_t i = 0; i < flows.size(); i++) {
    ...
    if (f->size_in_pkt == 3) {
      ...
      if (freq.find(fct) == freq.end()) freq[fct] = 0;
      freq[fct]++;
    }
  }
...
void assign_flow_deadline(std::deque<Flow *> flows) {
  ExponentialRandomVariable *nv_intarr =
      new ExponentialRandomVariable(params.avg_deadline);
  for (uint i = 0; i < flows.size(); i++) {
    ...
    // std::cout << f->start_time << " " << f->deadline << " " <<
    // topology->get_oracle_fct(f)/1000000 << " " << rv << "\n";
  }
}
...
void printQueueStatistics(Topology *topo) {
  ...
  std::cout << " Overall:" << std::setprecision(2)
            << (double)total_drop * 1460 / totalSentFromHosts << "\n";

  double totalSentToHosts = 0;
  for (auto tor = (topo->switches).begin(); tor != (topo->switches).end();
       tor++) {
...
void run_experiment(int argc, char **argv, uint32_t exp_type) {
  ...
  if (params.flow_type == FASTPASS_FLOW) {
    topology = new FastpassTopology(params.num_hosts, params.num_agg_switches,
                                    params.num_core_switches, params.bandwidth,
                                    params.queue_type);
  } else if (params.big_switch) {
    topology = new BigSwitchTopology(params.num_hosts, params.bandwidth,
                                     params.queue_type);
  } else {
  ...
  if (params.use_flow_trace) {
    ...
    fg = new CustomCDFFlowGenerator(
        num_flows, topology, params.cdf_or_flow_trace, params.interarrival_cdf);
    fg->make_flows();
  } else if (params.permutation_tm != 0) {
    fg = new PermutationTM(num_flows, topology, params.cdf_or_flow_trace);
    fg->make_flows();
  } else if (params.bytes_mode) {
    fg = new PoissonFlowBytesGenerator(num_flows, topology,
                                       params.cdf_or_flow_trace);
    fg->make_flows();
  } else if (params.traffic_imbalance < 0.01) {
    fg =
        new PoissonFlowGenerator(num_flows, topology, params.cdf_or_flow_trace);
    fg->make_flows();
  } else {
    // TODO skew flow gen not yet implemented, need to move to FlowGenerator
    assert(false);
    // generate_flows_to_schedule_fd_with_skew(params.cdf_or_flow_trace,
    // num_flows, topology);
  }
  ...
  struct FlowComparator {
    bool operator()(Flow *a, Flow *b) { return a->start_time < b->start_time; }
  } fc;
  ...
  for (uint32_t i = 0; i < flows_sorted.size(); i++) {
    ...
    if (exp_type == GEN_ONLY) {
      std::cout << f->id << " " << f->size << " " << f->src->id << " "
                << f->dst->id << " " << 1e6 * f->start_time << "\n";
    } else {
  ...
  for (uint32_t i = 0; i < flows_sorted.size(); i++) {
    ...
    validate_flow(f);
    if (!f->finished) {
      f->log->end(true, active_flows, f->finish_time, f->cwnd_mss, f->rtt);
      std::cout << "unfinished flow "
                << "size:" << f->size << " id:" << f->id
                << " next_seq:" << f->next_seq_no
                << " recv:" << f->received_bytes << " src:" << f->src->id
                << " dst:" << f->dst->id << "\n";
    }
  }
